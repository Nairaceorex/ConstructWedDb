...
{
    public class FilterFinModel : PageModel
    {
        private readonly ConstructWedDb.Data.ConstructWedDbContext _context;
        public FilterFinModel(ConstructWedDb.Data.ConstructWedDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> OnGetAsync(bool? fin)
        {
            if (fin == null)
            {
                return NotFound();
            ...
            Order = await _context.Order.Where(m => m.CompletionMark == fin).ToListAsync();
